...
int main(int argc, char** argv)
{
	...
	//计算误差
	{
		...
		cout << "点云实际平移距离:\n" << TRANS_origin << endl;
		t_error_x = fabs(transformation_icp(0, 3)) - fabs(TRANS_origin(0));
		t_error_y = fabs(transformation_icp(1, 3)) - fabs(TRANS_origin(1));
		t_error_z = fabs(transformation_icp(2, 3)) - fabs(TRANS_origin(2));
		cout << "计算得到的平移距离" << endl << "x轴平移" << transformation_icp(0, 3) << endl << "y轴平移" << transformation_icp(1, 3) << endl << "z轴平移" << transformation_icp(2, 3) << endl;
